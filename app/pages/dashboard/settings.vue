<template>
  <div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-gray-100 p-6">
      <div v-if="loading && !settings" class="text-center py-16 text-gray-400">Caricamento…</div>

      <template v-else-if="settings">
        <div v-if="successMsg" class="mb-5 bg-emerald-50 text-emerald-800 rounded-xl p-3 text-sm font-medium">{{ successMsg }}</div>
        <div v-if="error" class="mb-5 bg-red-50 text-brand-red rounded-xl p-3 text-sm">{{ error }}</div>

        <form class="space-y-8" @submit.prevent="save">
          <!-- Pricing -->
          <fieldset class="space-y-4">
            <legend class="text-sm font-semibold text-navy-deep pb-2 border-b border-gray-100 w-full block mb-4">
              Tariffe
            </legend>
            ...
            <label class="flex items-center gap-3 text-sm text-gray-700 cursor-pointer">
              ...
              <span><span class="font-semibold">Applica tariffa giornaliera</span> <span class="text-gray-400">(usa la tariffa al giorno quando più conveniente)</span></span>
            </label>
            <label class="flex items-center gap-3 text-sm text-gray-700 cursor-pointer">
              <input v-model="form.enable_hourly_booking" type="checkbox" class="rounded border-gray-300 text-navy" />
              <span><span class="font-semibold">Abilita prenotazioni orarie</span> <span class="text-gray-400">(se disabilitato i clienti prenotano solo giornate intere)</span></span>
            </label>
          </fieldset>
          ...
          <fieldset class="space-y-4">
            ...
            <div>
              <label class="block text-xs font-medium text-gray-500 mb-1.5">Ore minime prima della prenotazione per il rimborso</label>
              <input
                v-model.number="form.cancellation_cutoff_hours"
                type="number"
                min="0"
                required
                class="w-40 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy"
              />
              <p class="text-xs text-gray-400 mt-1">Il rimborso è automatico solo se mancano almeno N ore all'inizio della prenotazione.</p>
            </div>
          </fieldset>
          ...
          <div class="pt-2">
            <button
              type="submit"
              :disabled="saving"
              class="px-6 py-2.5 bg-navy text-white text-sm font-semibold rounded-lg hover:bg-navy-deep disabled:opacity-50 transition-colors"
            >
