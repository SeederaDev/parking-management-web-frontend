<template>
  <div class="max-w-2xl mx-auto px-6 py-16 text-center">
    ...
    <template v-else-if="paid">
      ...
      <h1 class="text-2xl font-bold text-gray-800 mb-2">Pagamento confermato!</h1>
      <p class="text-gray-400 mb-8">La tua prenotazione è stata pagata con successo.</p>
      ...
      <div v-if="booking" class="bg-white border border-gray-200 rounded-2xl p-6 mb-8 text-left text-sm">
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="font-semibold text-gray-800">{{ booking.spot_identifier }} — {{ booking.location_name }}</div>
          <div class="text-gray-400 uppercase tracking-wide text-xs">Ingresso</div>
          <div class="font-semibold text-gray-800">{{ formatDt(booking.start_time) }}</div>
          <div class="text-gray-400 uppercase tracking-wide text-xs">Uscita</div>
          <div class="font-semibold text-gray-800">{{ formatDt(booking.end_time) }}</div>
          <div class="text-gray-400 uppercase tracking-wide text-xs">Totale pagato</div>
    ...
    <template v-else-if="pollingTimeout">
      <div class="text-6xl mb-6">⏳</div>
      <h1 class="text-2xl font-bold text-gray-800 mb-2">Pagamento in elaborazione</h1>
    ...
    <template v-else-if="error">
      <div class="text-6xl mb-6">❌</div>
      <h1 class="text-2xl font-bold text-gray-800 mb-2">Errore nella verifica</h1>
      <p class="text-gray-400 mb-8">{{ error }}</p>
      <NuxtLink to="/booking" class="inline-block bg-indigo-600 text-white font-bold px-8 py-3.5 rounded-xl hover:bg-indigo-700 transition-colors uppercase tracking-widest text-sm">
